...
int SSLClientSocketImpl::Init() {
  ...
  ///// Add for TTPA /////
  //SSL_enable_ttpa(ssl_.get());

  ///// Add for MB /////
  SSL_enable_mb(ssl_.get());
...
///// Add for MB /////
int SSLClientSocketImpl::DoVerifyCertMB(int result) {
  DCHECK(start_cert_verification_time_.is_null());

  SSL* ssl = ssl_.get();
  uint8_t num_keys = SSL_get0_num_keys(ssl);
  int ret = OK;
  ...
  // Note: is this fine to pull this out of loop?
  next_handshake_state_ = STATE_VERIFY_CERT_COMPLETE;
  return ret;
}
