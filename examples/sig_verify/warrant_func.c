...
int make_warrant_content_body(unsigned char **content, EVP_PKEY *alice_pub, EVP_PKEY *carol_pub, int nid, int *len)
{
	...
	*len = sizeof(ts) + sizeof(duration) + sizeof(ht) + shalen;
	printf("PROGRESS: length of warrant_content_body: %d\n", *len);
	*content = (unsigned char *)OPENSSL_zalloc(*len);
	...
	printf("PROGRESS: print warrant_content_body\n");
	for (i=0; i<*len; i++)
	{
		if (i % 10 == 0)
			printf("\n");
...
// Output
//   Success: 1, Failure: 0
int make_warrant_request(unsigned char **request, unsigned char *msg, int msg_len, EVP_PKEY *carol_priv, int nid, int *len)
{
	...
	printf("PROGRESS: make the signature block for the warrant content success\n");

	*len = sizeof(uint16_t) + msg_len + sigblk_len;

	printf("Length of warrant request: %d\n", *len);
...
int make_warrant_request_with_verify_warrant(unsigned char **request, unsigned char *msg, int msg_len, EVP_PKEY *carol_priv, EVP_PKEY *alice_pub, EVP_PKEY *carol_pub, int nid, int *len)
{
	if (!verify_warrant_content_body(msg, alice_pub, carol_pub))
	{
		printf("ERROR: Verify warrant content body failure in make_warrant_request_with_verify_warrant\n");
		return 0;
	}
	return make_warrant_request(request, msg, msg_len, carol_priv, nid, len);
}
...
//   Success 1
//   Failure 0
int make_warrant_response(unsigned char **response, unsigned char *request, int req_len, EVP_PKEY *alice_priv, int nid, int *len)
{
	unsigned char *sigblk, *p;
	size_t sigblk_len;

	printf("PROGRESS: Make the warrant response\n");
	...
	*len = sizeof(uint16_t) + req_len + sigblk_len;
	...
	printf("Length of warrant response: %d\n", *len);
...
int make_warrant_response_with_verify_request(unsigned char **response, unsigned char *request, int req_len, EVP_PKEY *alice_priv, EVP_PKEY *alice_pub, EVP_PKEY *carol_pub, int nid, int *len)
{
	if (!verify_warrant_request(request, alice_pub, carol_pub))
	{
		printf("ERROR: Verify the warrant request failed in make_warrant_response_with_verify_request\n");
		return 0;
	}
	printf("PROGRESS: Verify the warrant request success in make_warrant_response_with_verify_request\n");

	return make_warrant_response(response, request, req_len, alice_priv, nid, len);
}
...
// Output
//   Success: 1, Failure: 0
int verify_warrant_content_body(unsigned char *content, EVP_PKEY *alice_pub, EVP_PKEY *carol_pub)
{
...
// Output
//   Success 1, Failure 0
int verify_warrant_request(unsigned char *request, EVP_PKEY *alice_pub, EVP_PKEY *carol_pub)
{
	int i;
	size_t len = 0;
	...
	printf("PROGRESS: Invoke verify_warrant_request()\n");
	...
	if (!verify_signature(warrant, len, sig_type, sig_len, sig, carol_pub))
	{
	...
	if (!verify_warrant_content_body(warrant, alice_pub, carol_pub))
	{
		printf("ERROR: Verify warrant content body error\n");
		goto err;
	}

	printf("PROGRESS: Verify warrant content body success\n");
...
int verify_warrant_response(unsigned char *response, EVP_PKEY *alice_pub, EVP_PKEY *carol_pub)
{
	int i;
	size_t len = 0;
	uint16_t sig_type, sig_len;
	...
	printf("PROGRESS: Invoke verify_warrant_response()\n");

	p = response;
	n2s(p, len);
	printf("PROGRESS: Length of warrant_response: %d\n", (int)len);
	...
	{
		printf("ERROR: Verify the warrant request signature error\n");
		return 0;
	}
	printf("PROGRESS: Verify the warrant request signature success\n");
