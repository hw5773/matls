...
#include "mssl/include/func.h"
#include "mssl/include/sp.h"
#include "mssl/include/record_header.h"
...
static void https_message_process(mctx_t mctx, int sock, int side, uint64_t events, filter_arg_t *arg)
{
  ...
  {
    struct recordhdr *rh;
    rh = (struct recordhdr *) p.payload;
    printf("Content Type: %s\n", get_content_type_string(get_content_type_idx(rh->content_type)));
    printf("Major: %x\n", rh->major);
    printf("Minor: %x\n", rh->minor);
  }

//  fprintf(stderr, "[mOS] %s: Content Type: %x\n", __func__, rh->content_type);
}
