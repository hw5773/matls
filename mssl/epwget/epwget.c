...
CreateConnection(thread_context_t ctx)
{
	...
	errno = 0;
	sockid = mssl_socket(mctx, AF_INET, SOCK_STREAM, 0);
	if (sockid < 0) {
		return -1;
	}
	memset(&ctx->wvars[sockid], 0, sizeof(struct wget_vars));
	ret = mssl_setsock_nonblock(mctx, sockid);
	if (ret < 0) {
		exit(-1);
	}
	...
	ret = mssl_connect(mctx, sockid, 
			(struct sockaddr *)&addr, sizeof(struct sockaddr_in));
	if (ret < 0) {
		if (errno != EINPROGRESS) {
			perror("mssl_connect");
			mssl_close(mctx, sockid);
			return -1;
		}
	}
	...
	ev.data.sock = sockid;
	mssl_epoll_ctl(mctx, ctx->ep, MOS_EPOLL_CTL_ADD, sockid, &ev);
...
CloseConnection(thread_context_t ctx, int sockid)
{
	...
	while (/*ctx->pending*/ mssl_get_connection_cnt(ctx->mctx) < concurrency && ctx->started < ctx->target) {
...
SendHTTPRequest(thread_context_t ctx, int sockid, struct wget_vars *wv)
{
	...
	wr = mssl_write(ctx->mctx, sockid, request, len);
	if (wr < len) {
...
{	
	...
	ev.events = MOS_EPOLLOUT;
	ev.data.sock = sockid;
	mssl_epoll_ctl(mctx, ctx->ep, MOS_EPOLL_CTL_MOD, sockid, &ev);	
...
HandleReadEvent(thread_context_t ctx, int sockid, struct wget_vars *wv)
{
	...
	rd = 1;
	while (rd > 0) {
		rd = mssl_read(mctx, sockid, buf, BUF_SIZE);
		if (rd <= 0)
			break;
		ctx->stat.reads += rd;
...
InitWget(mctx_t mctx, void **app_ctx)
{
	...
	mssl_init_rss(mctx, saddr, IP_RANGE, daddr, dport);
	...
	ep = mssl_epoll_create(mctx, ctx->maxevents);
	if (ep < 0) {
		exit(EXIT_FAILURE);
	}
	ctx->events = (struct mssl_epoll_event *)
			calloc(ctx->maxevents, sizeof(struct mssl_epoll_event));
	if (!ctx->events) {
		exit(EXIT_FAILURE);
	}
	ctx->ep = ep;
...
RunWget(mctx_t mctx, void **app_ctx)
{
	...
	while (!done[core]) {
		...
		nevents = mssl_epoll_wait(mctx, ctx->ep,
		...
		for (i = 0; i < nevents; i++) {
			...
			if (ctx->events[i].events & MOS_EPOLLERR) {
				...
				ctx->stat.errors++;
				ctx->errors++;
				if (mssl_getsockopt(mctx, ctx->events[i].data.sock, 
