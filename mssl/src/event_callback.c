...
/* FIXME: ft_map is not managed properly (especially in unreg_cb()). */
...
/*----------------------------------------------------------------------------*/
void
init_event(mssl_manager_t mssl, int num_evt)
{
	if (!(mssl->evt_pool = mp_create(sizeof(struct ev_table),
...
/*----------------------------------------------------------------------------*/
void
init_evp(struct ev_pointer *evp, struct ev_base *evb)
{
	*evp = evb->dflt_evp;
...
/*----------------------------------------------------------------------------*/
void
cleanup_evp(struct ev_pointer *evp)
{
...
/*----------------------------------------------------------------------------*/
void
init_evb(mssl_manager_t mssl, struct ev_base *evb)
{
	TAILQ_INIT(&evb->evth);
	struct ev_table *dflt_evt = mp_allocate_chunk(mssl->evt_pool);
...
/*----------------------------------------------------------------------------*/
void
cleanup_evb(mssl_manager_t mssl, struct ev_base *evb)
{
	struct ev_table *walk, *tmp;
	for (walk = TAILQ_FIRST(&evb->evth); walk != NULL; walk = tmp) {
		tmp = TAILQ_NEXT(walk, link);

		mp_free_chunk(mssl->evt_pool, walk);
	}
}
/*----------------------------------------------------------------------------*/
static inline void
reg_cb_wcpy(struct ev_pointer *evp, struct ev_table *new_evt,
...
/*----------------------------------------------------------------------------*/
static inline void
reg_cb_wocpy(struct ev_pointer *evp, event_t events, void *cb)
{
	/* NOTE: We may apply binary search which is O(log(N)) later, while current
	 * linear search is O(N). */
...
/*----------------------------------------------------------------------------*/
static inline void
unreg_cb(struct ev_pointer *evp, event_t events)
{
	assert(evp);
	 ...
	 * is for each event table, then we need to call find_reusable_evt() for
	 * every callback unregistration to find reusable event table.
	 * find_reusable_evt() is heavier than per-event reference counter update.
...
find_reusable_evt(struct ev_pointer *evp, struct ev_base *evb,
		event_t events, void *cb)
{
	struct ev_table *cur_evt = evp->evt;
...
/*----------------------------------------------------------------------------*/
inline int
mod_cb(mssl_manager_t mssl, int op, struct ev_pointer *evp, struct ev_base *evb,
		event_t events, void *cb)
{
	struct ev_table *evt = evp->evt;
	...
	if (op == OP_REG) {
		...
		if (events & evp->cb_map) {
			...
			if (!(nevt = find_reusable_evt(evp, evb, events, cb))) {
				nevt = mp_allocate_chunk(mssl->evt_pool);
				assert(nevt);
			...
			/* register callback */
			if (nevt != evt)
				reg_cb_wcpy(evp, nevt, events, cb);
			else
				reg_cb_wocpy(evp, events, cb);
...
/*----------------------------------------------------------------------------*/
static inline int
modify_callback(mctx_t mctx, int op, int sockid, event_t events,
		                       int hook_point, void* callback)
{
	...
	return mod_cb(mssl, op, evp, evb, events, callback);
}
/*----------------------------------------------------------------------------*/
int
mssl_register_callback(mctx_t mctx, int sockid, event_t events,
		                       int hook_point, callback_t callback)
{
	...
	return modify_callback(mctx, OP_REG, sockid, events, hook_point, callback);
}
/*----------------------------------------------------------------------------*/
int
mssl_unregister_callback(mctx_t mctx, int sockid, event_t events,
		                       int hook_point)
{
	return modify_callback(mctx, OP_UNREG, sockid, events, hook_point, NULL);
}
/*----------------------------------------------------------------------------*/
/*
event_t
mssl_define_event(event_t event, filter_t filter)
{
	int i, j;
	int evid;
 ...
 */
inline void
handle_callback(mssl_manager_t mssl, uint32_t hook, 
...
#define MLSNR(sock) (sock)->monitor_listener
	...
	mssl->pctx = pctx;
	mctx_t const mctx = g_ctx[mssl->ctx->cpu];
	...
	EVENT_FOREACH_START(bev, bidx, BEV_OFFSET, BEV_OFFSET + NUM_BEV, bev_map) {
		...
		while (1) {
			...
			if (cb_map & ev) {
				evt->ent[ev_idx].cb(mctx, socket->id, side, ev, data.ptr);
			...
			while (1) {
				...
				if (((1L << ude_id) & (cb_map | ft_map)) &&
					(tmpdata.u64 = g_udes[ude_id - UDE_OFFSET].ft(mctx, socket->id, side, ev, data.ptr))) {
