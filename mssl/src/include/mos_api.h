...
enum mos_event_type
{
	 ...
	 * This is implemented as mssl_cb_settimer()
	 */
 ...
 *
 * NOTE: When you retrieve the packet information using mssl_getlastpkt()
...
struct pkt_info {
	 ...
	 * of pctx during mssl_getlastpkt() invocation.
	 */
	struct ethhdr *ethh;
	struct iphdr  *iph;
	struct tcphdr *tcph;
 ...
 * See mssl_getsockopt() and mssl_setsockopt() the mssl_api.h file.
 */
 ...
 */
int
mssl_bind_monitor_filter(mctx_t mctx, int sock, monitor_filter_t ft);
/*----------------------------------------------------------------------------*/
 ...
 */
int
mssl_register_callback(mctx_t mctx, int sock, event_t event,
 ...
 */
//int
//mssl_unregister_callback(mctx_t mctx, int sock, event_t event,
//		                       int hook_point);
 ...
 */
event_t
mssl_alloc_event(event_t event);
 ...
 */
event_t
mssl_define_event(event_t event, filter_t filter, struct filter_arg *arg);
 ...
 */
int
mssl_raise_event(mctx_t mctx, event_t event);
 ...
 */
void
mssl_set_uctx(mctx_t mctx, int sock, void *uctx);

/** Get user-level context
 * (e.g., to retrieve user-defined metadata stored in mssl_set_uctx())
 * @param [in] mctx: mssl context
 * @param [in] sock: the monitor socket id
 * @return user-level context for input flow_ocntext
 */
void *
mssl_get_uctx(mctx_t mctx, int sock);
...
*/
ssize_t
mssl_peek(mctx_t mctx, int sock, int side,
	     char *buf, size_t len);

/**
 * The mssl_ppeek() function reads up to count bytes from the TCP ring
 ...
 * of bytes read from the buffer. After mssl_ppeek(), the data in the
 * TCP ring buffer will not be flushed, and the monitor offset used by
 * mssl_peek() is not changed.
 *
 * @param [in] mctx: mssl context
 * @param [in] sock: monitoring stream socket id
 * @param [in] side: side of monitoring (client side, server side or both)
 ...
 */
ssize_t mssl_ppeek(mctx_t mctx, int sock, int side, 
			  char *buf, size_t count, uint64_t off);

/* Use this macro to copy packets when mssl_getlastpkt is called */
#define mssl_CB_GETCURPKT_CREATE_COPY

/** Get current packet of mssl context
 * @param [in] mctx: mssl/mOS context
 * @param [in] sock: monitoring stream socket id
 ...
 */
int
mssl_getlastpkt(mctx_t mctx, int sock, int side, struct pkt_info *p);
 ...
 */
int
mssl_settimer(mctx_t mctx, int id, struct timeval *timeout, callback_t cb);

/** A sibling function to mssl_settimer that returns
...
/** Pause mssl application context since it is not running anything
 * @param [in] mctx: mssl context
 *
 * This is useful for running callback-only applications
 */
 ...
 */
int
mssl_getpeername(mctx_t mctx, int sock, struct sockaddr *saddr, socklen_t *addrlen, int side);
...
mssl_setlastpkt(mctx_t mctx, int sock, int side, off_t offset,
		uint8_t *data, uint16_t datalen, int option);

/** Send a TCP packet of struct pkt_info
 * @param [in] mctx: mssl/mOS context
 * @param [in] sock: monitoring stream socket id
 * @param [in] pkt: ptr to packet info (e.g., captured by mssl_getlastpkt)
 * @return 0 on success, -1 on failure
