...
int
get_buf_info(socket_map_t sock, int side, void *optval, socklen_t *optlen);

int
get_tcp_state(struct tcp_stream *stream, int side,
			void *optval, socklen_t *optlen);

int
disable_buf(socket_map_t sock, int side);

int
get_last_timestamp(struct tcp_stream *stream, uint32_t *usecs, socklen_t *sz);
