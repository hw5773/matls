...
/*---------------------------------------------------------------------------*/
int 
mssl_pipe(mctx_t mctx, int pipeid[2])
{
	socket_map_t socket[2];
	struct pipe *pp;
	int ret;
	
	socket[0] = allocate_socket(mctx, MOS_SOCK_PIPE);
	if (!socket[0]) {
		errno = ENFILE;
		return -1;
	}
	socket[1] = allocate_socket(mctx, MOS_SOCK_PIPE);
	if (!socket[1]) {
		free_socket(mctx, socket[0]->id, MOS_SOCK_PIPE);
		errno = ENFILE;
		return -1;
	}
	...
	if (!pp) {
		/* errno set by calloc() */
		free_socket(mctx, socket[0]->id, MOS_SOCK_PIPE);
		free_socket(mctx, socket[1]->id, MOS_SOCK_PIPE);
		return -1;
	}
...
/*---------------------------------------------------------------------------*/
static void 
raise_event_to_pair(mssl_manager_t mssl, socket_map_t socket, uint32_t event)
{
	...
	if (pair_socket->opts & mssl_NONBLOCK) {
		if (pair_socket->epoll) {
			add_epoll_event(mssl->ep, USR_EVENT_QUEUE, pair_socket, event);
		}
	} else {
...
PipeRead(mctx_t mctx, int pipeid, char *buf, int len)
{
	...
	if (to_notify) {
		raise_event_to_pair(mssl, socket, MOS_EPOLLOUT);
	}
	...
	if (pp->buf_len > 0) {
		...
		add_epoll_event(mssl->ep, USR_SHADOW_EVENT_QUEUE, socket, MOS_EPOLLIN);
	}
...
pipe_write(mctx_t mctx, int pipeid, const char *buf, int len)
{
	...
	if (to_notify) {
		raise_event_to_pair(mssl, socket, MOS_EPOLLIN);
	}
...
{
	struct mssl_epoll *ep = get_socket(mctx, epid)->ep;
	socket_map_t socket = get_socket(mctx, pipeid);
	struct pipe *pp = socket->pp;
