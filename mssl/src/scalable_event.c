 ...
 * */
inline int
reg_cb(kvs_t *store, stree_t **pstree, event_t ev, callback_t cb)
{
#ifdef DBGMSG
 ...
 * */
inline int
unreg_cb(kvs_t *store, stree_t **pstree, event_t ev)
{
#ifdef DBGMSG
	__PREPARE_DBGLOGGING();
#endif
...
/*----------------------------------------------------------------------------*/
inline int
mod_cb(kvs_t *store, stree_t **pstree, event_t ev, callback_t cb)
{
	assert(*pstree || cb);
	...
	if (cb)
		return reg_cb(store, pstree, ev, cb);
	else
		return unreg_cb(store, pstree, ev);
}
/*----------------------------------------------------------------------------*/
void
...
inline int
raise_ev(kvs_t *store, event_t event)
{
	tree_node_t *ev = NULL;
#if 1
	struct {
...
mtcp_raise_event(mctx_t mctx, event_t event)
{
	...
	return raise_ev(mtcp->ev_store, event);
}
/*----------------------------------------------------------------------------*/
#define PRINT_EV(event, field)						\
	printf("[%s](node: %p) ev: %ld, older: %ld, younger: %ld, child: %ld, " \
	       ...
	       (event)->ft, (event)->cb)
inline void
handle_cb(mctx_t const mctx, int sock, int side,
					   ...
					   int hook_point, callback_t callback)
{
	...
	return mod_cb(mtcp->ev_store, pstree, event, callback);
}
/*----------------------------------------------------------------------------*/
int
...
#define MLSNR(sock) (sock)->monitor_listener
	...
	*/
	...
	handle_cb(mctx, socket->id, side, stree, events);
}
/*----------------------------------------------------------------------------*/
#endif
