...
tag_dic = {
        'CLIENT_TCP_CONNECT_START': 9,
        'CLIENT_TCP_CONNECT_END': 10,
        'CLIENT_HANDSHAKE_START': 1, 
        'CLIENT_EXTENDED_FINISHED_START': 7,
        'CLIENT_EXTENDED_FINISHED_END': 8,
        'CLIENT_HANDSHAKE_END': 2,
        'CLIENT_CERT_VALIDATION_START': 3, 
        'CLIENT_CERT_VALIDATION_END': 4,
        'CLIENT_MODIFICATION_RECORD_START': 11,
        'CLIENT_MODIFICATION_RECORD_END': 12,
        'CLIENT_FETCH_HTML_END': 6
}
...
if __name__ == '__main__':
    d = 'csv_data'
    for i in range(0, 17):
        result_fp = open('mb_%d_split_result.csv' % i, 'w')
        file_path = op.join(d, 'mb_%d_split' % i, 'mb_%d_split.csv' % i)
